...
enum abstract Action(String) to String from String
{
	...
	var CAMERA = 'camera';
	var LIGHT = 'light';
	var LIGHT_P = 'light-press';
	var LIGHT_R = 'light-release';
	var STILL = 'still';
	var STILL_P = 'still-press';
	var STILL_R = 'still-release';
}
#else
@:enum
...
class Controls extends FlxActionSet
{
	...
	var _camera = new FlxActionDigital(Action.CAMERA);
	var _light = new FlxActionDigital(Action.LIGHT);
	var _lightP = new FlxActionDigital(Action.LIGHT_P);
	var _lightR = new FlxActionDigital(Action.LIGHT_R);
	var _still = new FlxActionDigital(Action.STILL);
	var _stillP = new FlxActionDigital(Action.STILL_P);
	var _stillR = new FlxActionDigital(Action.STILL_R);
	...
	public var CAMERA(get, never):Bool;

	inline function get_CAMERA()
		return _camera.check();

	public var LIGHT(get, never):Bool;

	inline function get_LIGHT()
		return _light.check();

	public var LIGHT_P(get, never):Bool;

	inline function get_LIGHT_P()
		return _lightP.check();

	public var LIGHT_R(get, never):Bool;

	inline function get_LIGHT_R()
		return _lightR.check();

	public var STILL(get, never):Bool;

	inline function get_STILL()
		return _still.check();

	public var STILL_P(get, never):Bool;

	inline function get_STILL_P()
		return _stillP.check();

	public var STILL_R(get, never):Bool;

	inline function get_STILL_R()
		return _stillR.check();
	...
	public function setNewHitBox(Hitbox:FlxNewHitbox)
	{
		...
		inline forEachBound(Control.CAMERA, (action, state) -> addButtonNOTES(action, Hitbox.buttonSpace, state));
		inline forEachBound(Control.LIGHT, (action, state) -> addButtonNOTES(action, Hitbox.buttonShift, state));
		inline forEachBound(Control.STILL, (action, state) -> addButtonNOTES(action, Hitbox.buttonCtrl, state));
	}
	...
	function forEachBound(control:Control, func:FlxActionDigital->FlxInputState->Void)
	{
		switch (control)
		{
			...
			case CAMERA:
				func(_camera, JUST_PRESSED);
			case LIGHT:
				func(_light, PRESSED);
				func(_lightP, JUST_PRESSED);
				func(_lightR, JUST_RELEASED);
			case STILL:
				func(_still, PRESSED);
				func(_stillP, JUST_PRESSED);
				func(_stillR, JUST_RELEASED);
		}
	}
	...
	public function setKeyboardScheme(scheme:KeyboardScheme, reset = true)
	{
		...
		switch (scheme)
		{
			case Solo:
				...
				inline bindKeys(Control.CAMERA, [SPACE]);
				inline bindKeys(Control.LIGHT, [SHIFT]);
				inline bindKeys(Control.STILL, [CONTROL]);
			case Duo(true):
		...
		switch (scheme)
		{
			case Solo:
				...
				bindKeys(Control.CAMERA, [SPACE]);
				bindKeys(Control.LIGHT, [SHIFT]);
				bindKeys(Control.STILL, [CONTROL]);
			case Duo(true):
	...
	public function addDefaultGamepad(id):Void
	{
		...
		addGamepadLiteral(id, [
			...
			Control.CAMERA => [X],
			Control.LIGHT => [LEFT_SHOULDER],
			Control.STILL => [RIGHT_SHOULDER]
		]);
		#else
		addGamepadLiteral(id, [
